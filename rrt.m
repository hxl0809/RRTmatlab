...
motionPrimitiveCommandArray = [1 0; 1 1.3; 1 -1.3; 0.8 0.3; 0 0];
delta=0.25;
...
% for i=1:I
    ...
    vnew=extend(vnear,qrand,motionPrimitiveCommandArray,delta);
    
    
    status=checkedge(obs,vnew);
    if(status == -1)
        disp('collision')
    ...
    tau(addedVert).id=vnew.id;
    tau(addedVert).pose=vnew.pose;
    tau(addedVert).edgeq=vnew.edgeq;
    tau(addedVert).edgeu=vnew.edgeu;
    tau(addedVert).pid=vnew.pid;
    
    
    
    %% plot the new edge
    if(PRINTITER)
    figure(10),plot(vnew.edgeq(:,1),vnew.edgeq(:,2))
    end
